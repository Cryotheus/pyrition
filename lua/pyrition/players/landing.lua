...
local function claculate_radian_max(radius, radian_increment, first_apparition_radius) return tau - (first_apparition_radius / radius + radian_increment) end
...
local function find_suitable_landings(target, apparitions, force)
	...
	for apparition_index, apparition in ipairs(apparitions) do
		...
		local radian_max = claculate_radian_max(radius, radian_increment, first_apparition_radius)
		...
		for attempt = 1, max_attempts do
			...
			if radian >= radian_max then
				...
				radian_max = claculate_radian_max(radius, radian_increment, first_apparition_radius)
...
--pyrition functions
function PYRITION:PyritionPlayerFindLanding(target, apparitions) return find_suitable_landings(target, apparitions) end